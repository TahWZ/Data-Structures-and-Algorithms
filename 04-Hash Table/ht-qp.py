...
class HashTable: #Quadratic Probing
    ...
    def __getitem__(self, key):
        ...
        for _ in range(len(self.table)):
            if self.table[hash_val] is None:    #If the key value is not in table
                ...
                #---------------Task 4--------------
                hash_val = (N + s**2) % len(self.table) #Recalculate the hash_val
                s += 1
                #---------------Task 4--------------
...
def table_load_dictionary_statistics(max_time):
    ...
    #---------------Task 4--------------
    with open('output_task4.csv',mode='w',newline='') as csvfile:   #Creates/write a csvfile
    #---------------Task 4--------------
...
def self_test():
    try:
        tests = [test_init(),test_hash(),
             ...
             test_setitem(),test_rehash(),test_statistics(),
